...
class PedidoClienteDAO{
    ...
    async verificarPedido(cliente){
        ...
        const sentencia = async function(conexion){
            return await conexion.execute(
                `SELECT pcli.f_fechacreacion, pcli.f_fechamodificacion, pcli.v_subtotal, pcli.q_numeroproductos, pcli.v_iva, pcli.v_valortotal, pcli.k_idpedidocliente
                FROM pcli
    ...
    async cargarProductos(pedido){
        ...
        const sentencia = async function(conexion){
            return await conexion.execute(
                `SELECT prod.k_idreferenciaunica, prod.n_nombre, prod.n_marca, prod.k_idcategoria, prod.n_unidadmedida, prod.l_imagen, prodpcli.k_idproductopedidocliente, prodpcli.q_cantidad, prodpcli.v_precio
                FROM pcli, prodpcli, prod
                WHERE pcli.k_idpedidocliente = prodpcli.k_idpedidocliente AND prodpcli.k_idreferenciaunica = prod.k_idreferenciaunica AND pcli.k_idpedidocliente = :pedidocliente`,
                [pedido]
            );
        };
    ...
    async crearPedido(cliente/*,ciudad*/){
        ...
        const sentencia = async function(conexion){
            return await conexion.execute(
                `INSERT INTO pcli (F_FECHACREACION, F_FECHAMODIFICACION, I_ESTADO, V_SUBTOTAL, Q_NUMEROPRODUCTOS, K_IDCLIENTE, V_VALORTOTAL, V_IVA) 
                VALUES (TO_DATE(sysdate), TO_DATE(sysdate), 'INICIADO', 0, 0, :idcliente, 0, 0)`,
                [ cliente ],
                { autoCommit: true }
            );
        };
